    ...
    <section className="relative -mt-1 bg-white">
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="flex items-center justify-center">
          <JourneyStep number="1" label="Learn" active />
          <JourneyLine />
          <JourneyStep number="2" label="Practice" />
          <JourneyLine />
          <JourneyStep number="3" label="Communicate" />
        </div>
      </div>
    </section>
  );
...
function JourneyLine() {
  return <div className="w-8 sm:w-16 h-px bg-gray-200 mx-2 sm:mx-4" />;
}
    ...
    <article className="group bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-violet-100/70 hover:-translate-y-2 transition-all duration-300">
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-5 left-5">
          <span className="inline-block bg-white/90 backdrop-blur text-violet-700 text-xs font-bold tracking-[0.15em] px-3 py-1.5 rounded-lg shadow-lg">
            {level.eyebrow}
          </span>
        </div>
      </div>
...
function Footer({ onSelectLevel, onDictionary, onAbout }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <FooterColumn title="Learning">
            <FooterButton onClick={() => onSelectLevel("beginner")}>
              Beginner
            </FooterButton>
            <FooterButton onClick={() => onSelectLevel("intermediate")}>
              Intermediate
            </FooterButton>
            <FooterButton onClick={() => onSelectLevel("advanced")}>
              Advanced
            </FooterButton>
          </FooterColumn>
